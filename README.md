...
**URL:** 
https://reporting.angelo-j.xyz
https://test.angelo-j.xyz
https://collector.angelo-j.xyz/
https://angelo-j.xyz/
...
All models receive a shared PDO instance from `api/db.php`. The three analytics models (`Static`, `Performance`, `Activity`) are read-only from the web app side — writes come exclusively through the API layer.
...
- `layout/header.php` — Opens the HTML document, loads Chart.js from CDN, links the stylesheet, and renders the nav bar (conditionally gated by role and section permissions)
- `layout/footer.php` — Closes HTML tags and includes the `exportWithChart()` JS helper for PDF/image export
- `layout/pagination.php` — Reusable Previous/Next controls, expects `$page`, `$totalPages`, `$baseUrl` from the including view
...
- Pagination (this is done already)
...
2. **Rate Limiting** — Add request rate limiting to the login endpoint and all API endpoints. Prevents brute-force password attacks and API abuse/DoS.
   - See: `PLAN_RATE_LIMITING.md`
...
4. **API Pagination & Database Indexing** — Add `limit`/`offset` pagination to all API endpoints and add database indexes on frequently queried columns (`session_id`, `network_type`, `type`). Prevents memory exhaustion on large datasets and speeds up aggregation queries.
   - See: `PLAN_API_PAGINATION_INDEXING.md`

---

### Extra Credit Features

1. **PDF Report Export** — Allow users to export any report page or saved report as a downloadable PDF using the already-installed dompdf library.
   - See: `PLAN_PDF_EXPORT.md`

2. **CSV Data Export** — Add a "Download CSV" button to each data table page so analysts can export raw data for offline analysis in Excel/Google Sheets.
   - See: `PLAN_CSV_EXPORT.md`

3. **Date Range Filters** — Add date range picker inputs to report pages so users can filter data by time window instead of always seeing all records.
   - See: `PLAN_DATE_RANGE_FILTERS.md`

4. **Dark Mode Toggle** — Add a user-togglable dark/light theme preference that persists via localStorage, improving accessibility and user comfort.
   - See: `PLAN_DARK_MODE.md`

5. **User Signup** — Public registration page where new users can create an account (username + password). Defaults to `viewer` role with access to saved reports only.
   - See: `PLAN_USER_SIGNUP.md`

6. **Session Activity Replay** — Visual timeline-based playback of user sessions. Select a session and watch clicks, scrolls, keyboard events, and idle periods animate on a canvas representation of the page, with a scrubable timeline, speed controls, and event log.
   - See: `PLAN_SESSION_REPLAY.md`
